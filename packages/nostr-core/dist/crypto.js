 ...
 */
export async function decryptPrivateKey(encryptedKeys, password) {
    // Convert from base64
    const encryptedBuffer = Uint8Array.from(atob(encryptedKeys.encryptedPrivateKey), c => c.charCodeAt(0));
    const salt = Uint8Array.from(atob(encryptedKeys.salt), c => c.charCodeAt(0));
